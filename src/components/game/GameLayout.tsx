 ...
 * Game Layout
 *
 * Provides consistent layout for both legacy F4T and unified architecture.
 * Detects which architecture to use and renders appropriate modals.
 */
...
function UnifiedArchitectureLayout() {
  ...
  // Show result modal when game is complete
  useEffect(() => {
  ...
  return (
    <main className="p-4 sm:p-6 max-w-full sm:max-w-xl mx-auto flex flex-col min-h-screen">
      ...
      {/* Back to Today button when in archive mode */}
      {isArchiveMode && gameConfig && (
        <div className="mt-4 mb-2">
          <Button
            onClick={() => exitUnifiedArchiveMode()}
            variant="outline"
            size="sm"
            className="w-full"
          >
...
/**
 * Legacy layout for F4T game
 */
function LegacyFFTLayout() {
  const {
    gamePhase,
    activePhase,
    modalVisible,
